  ...
  Backbone.DittoModel = Backbone.Model.extend
    ...
    toJSON: ->
      ...
      _.each json, (value, key) =>
        ...
        json[key] = value.toJSON() if isReference(value)
      json

    parse: (res) ->
      res = res[0] if _.isArray res
      _.each res, (val, key) => res[key] = @_build val if isReference val
      res

    _build: (data) ->
      _.each attrs = data.attributes, (val, key) =>
        if isReference val then attrs[key] = @_build val
